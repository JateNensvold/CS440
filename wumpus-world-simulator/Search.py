...
class SearchEngine:
    ...
    def FindPath(self, startLocation, startOrientation, goalLocation, goalOrientation):
        initialState = SearchState(
            startLocation, startOrientation, 0, None, Action.CLIMB)
        goalState = SearchState(
            goalLocation, goalOrientation, 0, None, Action.CLIMB)
    ...
    def AStarSearch(self, initialState, goalState):
        initialState.heuristic = self.CityBlockDistance(
            initialState.location, goalState.location)
        ...
        while (self.frontier):
            ...
            for action in [Action.GOFORWARD, Action.TURNLEFT, Action.TURNRIGHT]:
                ...
                if (childState):
                    self.nodeCount += 1
                    childState.heuristic = self.CityBlockDistance(
                        childState.location, goalState.location)
    ...
    def GetChildState(self, state, action):
        ...
        if (action == Action.TURNLEFT):
            childState = SearchState(
                state.location, state.orientation, state.depth + 1, state, Action.TURNLEFT)
        ...
        if (action == Action.TURNRIGHT):
            childState = SearchState(
                state.location, state.orientation, state.depth + 1, state, Action.TURNRIGHT)
            if (state.orientation == Orientation.UP):
                childState.orientation = Orientation.RIGHT
            if (state.orientation == Orientation.DOWN):
                childState.orientation = Orientation.LEFT
        ...
        if (action == Action.GOFORWARD):
            ...
            if (self.SafeLocation(x, y)):
                childState = SearchState(
                    [x, y], state.orientation, state.depth + 1, state, Action.GOFORWARD)
        return childState
